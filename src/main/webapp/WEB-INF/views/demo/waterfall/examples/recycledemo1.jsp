...
<div id="content">
    <div id="woo-holder">
        ...
        <a name="woo-anchor"></a>
        <ul id="switchholder">
            <li class="woo-swa woo-cur"><a href="${ctx}/admin/waterfall/recycledemo1">step1</a></li>
            <li><a href="${ctx}/admin/waterfall/recycledemo2">step2</a></li>
            <li><a href="${ctx}/admin/waterfall/recyclefinal">final</a></li>
        ...
        <div class="woo-swb">
            ...
            <div class="woo-pcont woo-masned my-pic" data-wootemp="2" data-subpagenum="40">
...
<script>
    $(function () {
        var conf = {
            ...
            // this function is not necessary in real environment
            "onLazyAddOver": function (pg, IDX) {
                // pg 为 Pagine 实例，pg.$pager 为底部翻页容器
                // idx 表示当前瀑布流的序号
                var unitcache = $.Woo.masn[IDX].unitCache;

                for (e in unitcache) {
                    var $ot = unitcache[e];
                    var htmlin = '<div title="' + $ot.data('idx') + '" style="height:' + $ot.data('ht') + 'px">' + $ot.data('idx') + '</div>';
                    unitcache[e].html(htmlin)
                }

                pg.$dom.find('.woo[data-idx]').each(function (i, e) {
                    var $ot = $(e),
                            htmlin = '<div title="' + $ot.data('idx') + '" style="height:' + $ot.data('ht') + 'px">' + $ot.data('idx') + '</div>';

                    unitcache[$ot.data('idx')] && (unitcache[$ot.data('idx')].html(htmlin))
                    $ot.html(htmlin)
                })
            },
    ...
    // override exRecycleInvisibleUnits to show the red units which should have been removed
    $.Woo.Masn.prototype.exRecycleInvisibleUnits = function (wt, domtp, rangeNum, posNum, isvNum) {
        var masn = this,
                startPos = 0,
                endPos = 0,
                nextRange;
